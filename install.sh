...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags "-s -w" -o "${APP_NAME}" ./cmd/cpu-alert
...
sudo install -m 0644 ./packaging/systemd/cpu-alert.service "${SERVICE_PATH}"
